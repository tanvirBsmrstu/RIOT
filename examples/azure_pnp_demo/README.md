...
1. along with the certificates, update ``AZ_RIOT_DPS_REGISTRATION_ID`` to the device ID you have chosen for device certificate.
    ``DAZ_RIOT_DPS_IDSCOPE`` to the ID scope of your DPS.
...
1. prerequisite : Since DNS64 is not working now, put the NAT64 IP of DPS in ``./modules/gnrc_wolfssl_tls/gnrc_wolfssl_tlc.c`` in function ``tcp_connect`` replace remoteIP variable.
...
Since DNS64 is not configured, put the NAT64 IP of IoT Hub in ``./modules/gnrc_wolfssl_tls/gnrc_wolfssl_tlc.c`` in function ``tcp_connect`` replace remoteIP variable. Then run the application again and follow the IP setup.
