                                    ...
                                    unsigned char *mqtts_readbuf, int mqtts_readbuf_len)
{
    ...
    int res = init_mqtts_layer(azContext, mqtts_writebuf, mqtts_writebuf_len, mqtts_readbuf,
                               mqtts_readbuf_len);
...
int init_dps_client(AzRiotContext *az_context)
{
    ...
    az_context->dps_client =
        (az_iot_provisioning_client *)malloc(sizeof(az_iot_provisioning_client));
...
int register_device_with_provisioning_service(AzRiotContext *azContext)
{
    ...
    return mqtts_publish(azContext->mqtts_context, register_topic_buffer,
                         (unsigned char *)mqtt_payload, 1, 0);
}
...
int subscribe_mqtt_client_to_provisioning_service_topics(AzRiotContext *azContext)
{
    ...
    int ret = mqtts_subscribe(azContext->mqtts_context,
                              AZ_IOT_PROVISIONING_CLIENT_REGISTER_SUBSCRIBE_TOPIC, 1);
...
int subscribe_mqtt_client_to_iot_hub_topics(AzRiotContext *azContext)
{
    ...
    ret = mqtts_subscribe(azContext->mqtts_context,
                          AZ_IOT_HUB_CLIENT_PROPERTIES_MESSAGE_SUBSCRIBE_TOPIC, 1);
    if (ret < 0) {
        printf("hub_example: Unable to subscribe to %s (%d)\n",
...
int handle_provisioning_message(char *topic, int topic_len, char *payload, int payload_len)
{
    ...
    if (az_result_failed(az_iot_provisioning_client_parse_received_topic_and_payload(azure_context->
                                                                                     dps_client,
                                                                                     topic_span,
                                                                                     message_span,
                                                                                     &
...
int handle_iot_message(char *topic, int topic_len, char *payload, int payload_len)
{
    ...
    if (az_result_succeeded(az_iot_hub_client_methods_parse_received_topic(azure_context->
                                                                           iot_hub_client,
                                                                           incoming_topic,
                                                                           &method_request))) {
    ...
    else if (az_result_succeeded(az_iot_hub_client_c2d_parse_received_topic(azure_context->
                                                                            iot_hub_client,
                                                                            incoming_topic,
                                                                            &c2d_request))) {
    ...
    else if (az_result_succeeded(az_iot_hub_client_twin_parse_received_topic(azure_context->
                                                                             iot_hub_client,
                                                                             incoming_topic,
                                                                             &twin_response))) {
