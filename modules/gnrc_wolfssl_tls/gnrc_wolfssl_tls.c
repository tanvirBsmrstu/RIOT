...
int tls_send(TLSContext *context, unsigned char *buffer, int buffer_len, int timeout_ms)
{
    ...
    {
        timeout_ms = timeout_ms < gnrc_wolfssl_tls_max_timeout_ms ? timeout_ms : gnrc_wolfssl_tls_max_timeout_ms;
    }
...
int tls_receive(TLSContext *context, unsigned char *buffer, int buffer_len, int timeout_ms)
{
    ...
    {
        context->timeout_ms = timeout_ms < gnrc_wolfssl_tls_max_timeout_ms ? timeout_ms : gnrc_wolfssl_tls_max_timeout_ms;
    }
...
int tcp_send(WOLFSSL *ssl, char *buf, int sz, void *ctx)
{
    ...
    if (!context)
    {
        tls_log("underlying socket can not be decided by wolfssl I/O callback due to Invalid socket.");
        return WOLFSSL_CBIO_ERR_GENERAL;
    }
...
int tcp_recv(WOLFSSL *ssl, char *buf, int sz, void *ctx)
{
    ...
    int timeout = context->timeout_ms == 0 ? context->timeout_ms_init : context->timeout_ms, recvLen = 0;
